...
小红书自动化工具箱，覆盖内容生产、发布、运营全流程。基于 Playwright 浏览器自动化，支持 MCP 协议与 AI Agent 集成。
...
1. **首次使用必须先登录**：运行 `xhs-kit login-browser` 弹出浏览器登录 或 `xhs-kit login-qrcode --terminal` 在终端中显示二维码登录
2. **检查登录状态**：运行 `xhs-kit status`（输出 `✅ 已登录` 或 `❌ 未登录`）
...
> - 或直接在命令行执行：`xhs-kit login-browser`
...
如果 `xhs-kit` 不在系统 PATH 中，使用完整路径：

```json
{
  "mcpServers": {
    "xiaohongshu": {
      "command": "/path/to/your/python/bin/xhs-kit",
      "args": ["serve"]
    }
  }
}
...
可通过 `which xhs-kit`（macOS/Linux）或 `where xhs-kit`（Windows）查看完整路径。
...
```bash
# 添加 MCP 服务
claude mcp add xhs-kit -- xhs-kit serve

# 查看已添加的 MCP
claude mcp list

# 移除 MCP 服务
claude mcp remove xhs-kit
```
...
```
封面文字：我开源了个小红书自动发布工具：xhs-kit

文字标题：我开源了个小红书自动发布工具：xhs-kit

文字正文内容：
我开源了个小红书自动发布工具：xhs-kit。

基于 Python，轻松兼容 Windows、Mac、Linux。
支持 Claude Code 和 OpenClaw，还能发布文字配图笔记。
核心原理：模拟浏览器登录。

项目地址：https://github.com/luweizheng/xhs-kit
...
# 发布图文
xhs-kit publish -t "标题" -c "文字正文内容" -i image1.jpg -i image2.jpg --tag 旅行 --tag 美食

# 发布视频
xhs-kit publish-video -t "标题" -c "文字正文内容" -v video.mp4

# 发布文字配图（将文字生成为卡片图片）
xhs-kit publish-text-card -c "封面文字" -p "第一页内容" -p "第二页内容" -s "基础" -t "笔记标题"

# 搜索内容
xhs-kit search -k "关键词"
...
**命令行：**
```bash
xhs-kit publish-text-card -c "封面文字" -p "第一页" -p "第二页" -s "基础" -t "笔记标题"
```
...
xhs-kit 采用模块化设计，当前包含以下模块：

- **`po` (Post & Operations)** - 运营模块
  - 发布：图文、视频、文字配图
  - 互动：点赞、收藏、评论、回复
  - 数据：搜索、推荐、笔记详情、用户主页
  - 登录：浏览器扫码、二维码登录
...
```bash
# 克隆项目
git clone https://github.com/luweizheng/xhs-kit.git
cd xhs-kit
...
- 浏览器扫码登录需要弹出浏览器窗口，请使用 `xhs-kit login-browser`
